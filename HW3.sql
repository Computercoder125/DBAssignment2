...
limit 1;                                                                           -- this ensures only the company with the highest yearly revenue
																		           -- is displayed
SELECT company_name, year, total_sales AS highest_annual_revenue              -- Selects the company name, year, and total sales as the highest annual revenue
FROM (
    SELECT m.name AS company_name, 
           YEAR(pl.order_date) AS year, 
           SUM(s.price * s.quantity_available) AS total_sales                            -- Inner query calculates total sales for each company per year
    FROM merchants m
